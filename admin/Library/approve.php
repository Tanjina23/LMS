...
<head>
  ...
  <title>Approved Book Request</title>
</head>
<style>
/* Side Navbar */
body {
...
<div class="container">
   <br><h3 class="py-2"style="text-align: center;">Approve Request</h3><br>
   ...
   <form class="approve" method="post"action="">
     ...
     <input class="form-control" type="text" name="return_book" placeholder="Return Date yyyy-mm-dd" ><br>
     <button class="btn btn-secondary" type="submit" name="submit">Approve</button>
    </form>
</div>
</div>
...
<?php
       ...
       {
          mysqli_query($connect,"UPDATE issue_book SET approve = '$_POST[approve]',
           issue = '$_POST[issue]', return_book = '$_POST[return_book]' WHERE email = '$_SESSION[email]'
           and bid = '$_SESSION[bid]';");

          mysqli_query($connect,"UPDATE displaybooks SET quantity = quantity-1 WHERE 
          bid = '$_SESSION[bid]';");

          mysqli_query($connect,"UPDATE displaybooks SET bcount = bcount+1 WHERE 
          bid = '$_SESSION[bid]';");

          $res = mysqli_query($connect,"SELECT quantity from displaybooks where 
          bid= '$_SESSION[bid]';");
          ...
          while($row=mysqli_fetch_assoc($res))
          {
              if($row['quantity'] == 0)
              {
                mysqli_query($connect,"UPDATE displaybooks SET status = 'Not-available'
