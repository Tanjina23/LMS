...
<br>
<main>
    <h2 class="py-2"style="text-align: center;">List of Books</h2>
<div class="container">
  <a style="text-decoration: none; font-size:25px;"href="manage_book.php">Add Book</a><br><br>
<table class="table table-striped table-hover table-responsive">
<thead class="table-info">
    <tr>
      <th width="8%"scope="col">S.No #</th>
      <th width="8%"scope="col">Category</th>
      <th width="15%"scope="col">Book Name</th>
      <th width="30%"scope="col">Details</th>
      <th width="10%"scope="col">Image</th>
      <th width="10%"scope="col">Added_on</th>
      <th width="35%"scope="col">Actions</th>
    </tr>
  </thead>
  <tbody>
    <?php
  //  for active deactive
    if(isset($_GET['type']) && $_GET['type']!== '' && isset($_GET['id']) && $_GET['id'] > 0){
        ...
        if($type == 'active' || $type == 'deactive'){
          ...
          mysqli_query($connect,"update book set book_status = '$book_status' where id ='$id'");
         //  redirect('category.php');
        }

    }

    $sql = "SELECT book.*,category.category from book,category where
     book.category_id = category.id   order by book.id desc";
    ...
    <tr>
      ...
      <td ><a target="_blank" href="<?php echo SITE_BOOK_IMAGE.$row['image']?>">
      <img style="height:50px; width:50px;" src="<?php echo SITE_BOOK_IMAGE.$row['image']?>" alt="book_image"/></a></td>
      <td >
        <?php
      ...
      <a href="?id=<?php echo $row['id']?>&type=deactive"><label class="badge bg-success btn ">
        ...
        ?>
        <a href="manage_book.php?id=<?php echo $row['id']?>"><label class="badge bg-info text-dark">Edit</label></a>&nbsp&nbsp
      </td>
    </tr>
   <?php 
   }
